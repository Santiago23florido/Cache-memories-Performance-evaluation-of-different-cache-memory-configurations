...
# Keys we must extract from stats.txt
REQUIRED_KEYS = [
    "simTicks",
    "simFreq",
...
class ResultRow:
    ...
    # Derived (latency in cycles, MPKI, delta CPI, percentages)
    l1_avg_miss_lat_cycles: float
    l2_avg_miss_lat_cycles: float
    mpki_l1: float
    mpki_l2: float

    delta_cpi_mem_total: float   # from L1D missLatency total (includes everything below it)
    delta_cpi_l2_miss: float     # from L2 missLatency total (deep misses beyond L2)
    delta_cpi_l1_only: float     # approx = total - l2_miss (avoid double count)
...
def parse_stats(stats_path: str) -> Dict[str, float]:
    """Parse a gem5 stats.txt and return only the needed key->value entries."""
...
def compute_one(case: str, stats_path: str) -> ResultRow:
    ...
    # Convert tick-based latencies to cycles using ticks_per_cycle.
    ...
    # Average miss latency in cycles (useful for interpretation)
    ...
    # Convert miss-latency totals (ticks) to total cycles "spent servicing misses"
    ...
    # Estimated delta CPI contributions (worst-case upper bound if no overlap)
    delta_cpi_mem_total = l1_miss_lat_cycles_total / numInsts
    delta_cpi_l2_miss = l2_miss_lat_cycles_total / numInsts

    # "Shared-effect" split to avoid double counting:
    # L1-total already includes deep misses; subtract L2-miss part to isolate L1-only (L2-hit) part.
    delta_cpi_l1_only = max(delta_cpi_mem_total - delta_cpi_l2_miss, 0.0)

    # Convert to percent of measured CPI
...
def to_csv(rows: List[ResultRow]) -> str:
    """CSV output for easy plotting/spreadsheets."""
    header = [
...
def to_latex(rows: List[ResultRow]) -> str:
    """A compact LaTeX table focusing on the CPI impact in percent."""
    out = []
    ...
    out.append(r"\begin{tabular}{l|c|c|c|c}")
    out.append(r"\hline")
    ...
    out.append(r"}")
    out.append(r"\caption{Estimated memory-penalty contribution to CPI for Dijkstra runs (gem5 miss-latency accounting).}")
...
def main() -> None:
    ...
    ap.add_argument("--out", default="dijkstra_mem_cpi.csv", help="CSV output filename.")
